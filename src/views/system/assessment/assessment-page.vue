...
const assessmentResult = ref({
  ...
  result: 'INFJ',
  result_detail:
    '你是一位务实、果断的人，喜欢组织和管理事务。你倾向于清晰的结构和明确的目标，擅长制定计划并执行。在团队中，你往往是可靠的领导者，能够有效地分配任务并确保项目按时完成。你重视效率和实际结果，不太喜欢模糊不清的情况。',
})
    ...
    <el-card class="assessment-card" style="margin-top: 20px">
      ...
      <template v-else-if="assessmentStep === 1">
        <el-row>
          ...
          <el-col :span="2"
            ><el-button link type="default" @click="assessmentStep = 0">取消</el-button></el-col
          >
        </el-row>
        <el-row class="assessment-row">
          <el-col>
            <el-progress
              :stroke-width="15"
              :text-inside="true"
              :percentage="((currentStep + 1) / quests.length) * 100"
            />
          </el-col>
        </el-row>
        <el-row class="assessment-row">
          <el-col
            ><h2 style="font-weight: 700">{{ quests[currentStep].content }}</h2></el-col
          >
        </el-row>
        ...
        <el-row class="assessment-row">
          <el-col :span="20">
            <el-button link type="default" @click="lastOne" :disabled="currentStep === 0"
              >上一题</el-button
            >
          </el-col>
          <el-col :span="4">
            <el-button type="primary" @click="nextOrSubmit" :disabled="currentAnswer === 0">{{
              currentStep + 1 === quests.length ? '提交答案' : '下一题'
            }}</el-button>
          </el-col>
        </el-row>
      </template>
...
.dot-radio,
.dot-radio /deep/ .el-radio__label {
  width: 100%;
  font-weight: 700;
  font-size: 1.1rem;
}
